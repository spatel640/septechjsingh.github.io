...
function isValidSearch(myCustomId) {
    var returnable = true;
    if (myCustomId.length != 16) {
        returnable = false;
    }
    if (myCustomId.length == 16) {
        myIDSub = myCustomId.substring(0, 10);
        if (myIDSub != "16DSNY-EW-")
            returnable = false;
    }
    return returnable;
}
    ...
    if (isValidSearch(myCustomId)) {
        ...
        $.ajax(settings).done(function (response) {
            ...
            if (response.result == undefined || response.result == "" || response.result == null) {
                alert("The SR# you entered no longer exists in the system.");
                mybool = false;
            }
        ...
        alert("Please enter an SR# in the format: 16DSNY-EW-XXXXXX");
    }
}
